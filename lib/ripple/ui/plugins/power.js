...
module.exports = {
    ...
    initialize: function () {
        var _RULES          =  constants.POWER_RULES,
            _DISPLAY_STATE  = constants.POWER_RESOURCE.SCREEN.STATE,
            displayObj       = document.getElementById("display-power-state"),
            brightnessLabel  = document.getElementById("display-brightness-label"),
            brightness, stateObj;
        ...
        function initPowerState() {
            ...
            //Init DISPLAY state.
            if (brightness ===  _DISPLAY_STATE.SCREEN_OFF.MAX) {
                displayObj.options[0].selected = true;
            } else if (brightness <= _DISPLAY_STATE.SCREEN_DIM.MAX) {
                displayObj.options[1].selected = true;
            } else if (brightness <= _DISPLAY_STATE.SCREEN_NORMAL.MAX) {
        ...
        $("#screen_off-rule-radio").buttonset();
        $("#screen_dim-rule-radio").buttonset();
        $("#screen_normal-rule-radio").buttonset();
        $("#screen_bright-rule-radio").buttonset();

        event.on("initPowerDB", function (rules) { //Update rule selects
            var obj;
            _RULES = rules;
            for (obj in rules) {
                stateObjId = obj.toLowerCase() + "-rule-radio";
                if (rules[obj]) {//Allow : true
                        stateObjId = stateObjId + "1";
                    } else {//Deny : false
                        stateObjId = stateObjId + "2";
                }
                $("#"+stateObjId).prop('checked', true);
            }
            $("#screen_off-rule-radio1").button("refresh");
            $("#screen_off-rule-radio2").button("refresh");
            $("#screen_dim-rule-radio1").button("refresh");
            $("#screen_dim-rule-radio2").button("refresh");
            $("#screen_normal-rule-radio1").button("refresh");
            $("#screen_normal-rule-radio2").button("refresh");
            $("#screen_bright-rule-radio1").button("refresh");
            $("#screen_bright-rule-radio2").button("refresh");
        });
        ...
        $("#screen_off-rule-radio1").click(function () {
            $("#screen_off-rule-radio2").prop('checked', false);
            $("#screen_off-rule-radio2").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_OFF', "value": true}]);
        });

        $("#screen_off-rule-radio2").click(function () {
            $("#screen_off-rule-radio1").prop('checked', false);
            $("#screen_off-rule-radio1").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_OFF', "value": false}]);
        });

        $("#screen_dim-rule-radio1").click(function () {
            $("#screen_dim-rule-radio2").prop('checked', false);
            $("#screen_dim-rule-radio2").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_DIM', "value": true}]);
        });

        $("#screen_dim-rule-radio2").click(function () {
            $("#screen_dim-rule-radio1").prop('checked', false);
            $("#screen_dim-rule-radio1").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_DIM', "value": false}]);
        });

        $("#screen_normal-rule-radio1").click(function () {
            $("#screen_normal-rule-radio2").prop('checked', false);
            $("#screen_normal-rule-radio2").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_NORMAL', "value": true}]);
        });

        $("#screen_normal-rule-radio2").click(function () {
            $("#screen_normal-rule-radio1").prop('checked', false);
            $("#screen_normal-rule-radio1").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_NORMAL', "value": false}]);
        });

        $("#screen_bright-rule-radio1").click(function () {
            $("#screen_bright-rule-radio2").prop('checked', false);
            $("#screen_bright-rule-radio2").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_BRIGHT', "value": true}]);
        });

        $("#screen_bright-rule-radio2").click(function () {
            $("#screen_bright-rule-radio1").prop('checked', false);
            $("#screen_bright-rule-radio1").button("refresh");
            event.trigger("updatePowerRules", [{"state": 'SCREEN_BRIGHT', "value": false}]);
        });
