...
module.exports = {
    ...
    initialize: function () {
        ...
        event.on("initPowerDB", function (rules) { //Update rule selects
            ...
            _RULES = rules;
            for (obj in rules) {
                stateObj = document.getElementById(obj.toLowerCase() + "-rule");
                if (stateObj) {
        ...
        document.getElementById("screen_off-rule").addEventListener("change", function () {
            var value = this.value;
            value = (value === '1' ? true : false);
            event.trigger("updatePowerRules", [{"state": 'SCREEN_OFF', "value": value}]);
        });
        document.getElementById("screen_dim-rule").addEventListener("change", function () {
            var value = this.value;
            value = (value === '1' ? true : false);
            event.trigger("updatePowerRules", [{"state": 'SCREEN_DIM', "value": value}]);
        });
        document.getElementById("screen_normal-rule").addEventListener("change", function () {
            var value = this.value;
            value = (value === '1' ? true : false);
            event.trigger("updatePowerRules", [{"state": 'SCREEN_NORMAL', "value": value}]);
        });
        document.getElementById("screen_bright-rule").addEventListener("change", function () {
            var value = this.value;
            value = (value === '1' ? true : false);
            event.trigger("updatePowerRules", [{"state": 'SCREEN_BRIGHT', "value": value}]);
        });
        initPowerState();
        initBattery();
    }
