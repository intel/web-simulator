...
function _setVehicleSupported (properties, supported) {
    ...
    for (i = 0; i < properties.length; i++) {
        ...
        hideHtml += '<input type="checkbox" id="' + property + '-supported" name="vehicle-supported" value="' + propertyAbbr + '" />';
...
function _setVehicleAutoRunning (autoRunning, isFill) {
    var autoContainer = jQuery("#vehicle-container #vehicle-running-show"),
        timeStamp, html = "", unit,  property, propertyItem, value;
    ...
    if (!isFill) {
        html += '<tr class="vehicle-property-content">' +
            ...
            '<td class="vehicle-property-value">' + timeStamp + '</td></tr>';

        for (property in autoRunning) {
            html += '<tr><td colspan="2" class="vehicle-property-header">' + property + '</td></tr>';

            for (propertyItem in autoRunning[property]) {
                value = autoRunning[property][propertyItem].value;
                unit = vehiclePanelEngine.getPropertyUnit(property, propertyItem);

                html += '<tr class="vehicle-property-content">' +
                    '<td class="vehicle-property-label">' + propertyItem + unit + '</td>' +
                    '<td class="vehicle-property-value">' + value + '</td>' +
                    '<td class="vehicle-property-value" style="display: none;">' +
...
function _initializeVehiclePanel () {
    try {
        var stop = false,
            configuration, current, properties, supported, settings, autoRunning;
        ...
        _setVehicleAutoRunning(autoRunning["auto0"], false);
