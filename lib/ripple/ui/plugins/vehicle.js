...
function _addVehicleEventListener () {
    event.on("vehicle-cache-request", function () {
        event.trigger("vehicle-cache-refresh",
            [
                _vehicleGlobalVars.vehicleCache.supported,
                _formatHistory(_vehicleGlobalVars.history[_vehicleGlobalVars.timeCurrent])
            ]
        );
    });
    ...
    event.on("vehicle-set-request", function (property, value) {
        ...
        try {
            for (var item in value) {
                ...
                enterContainer.val(value[item]);
                if (showContainer) {
                    showContainer.html(value[item]);
                }
            }
        }
        catch (err) {
