...
function _runVehicleInfo (manual) {
    ...
    tableString += '<tr><td align="left">Time</td><td id="vehicle-time" align="right">' + now + '</td></tr>';
        ...
        for (propertyItem in data[property]) {
            ...
            if (!manual) {
                tableString += '<tr><td align="left">' + propertyItem + _getPropertyUnit(property + '-' + propertyItem) +
                    '</td><td align="right" class="vehicle-property-auto" style="padding-right: 10px;">' + value +
                    '</td><td align="right" class="vehicle-property-manual" style="display: none;">' +
                    '<input style="text-align: right;" type="text" value="' + value +'" id="' + property + '-' + propertyItem + '-set">' +
                    '</td></tr>';
            }
            else {
                tableString += '<tr><td align="left">' + propertyItem + _getPropertyUnit(property + '-' + propertyItem) +
                    '</td><td align="right" class="vehicle-property-auto" style="display: none;">' + value +
    ...
    if (!manual) {
        ...
        _vehicleGlobal.oldManual = 0;
    }
    else {
        if (_vehicleGlobal.oldManual === 0) {
            content.html('');
            content.html(tableString);
            _setAutoPropertyValue();
            _vehicleGlobal.oldManual = 1;
        }
        else {
            $("#vehicle-time").html(now);
        }
    }
...
function _autoRunning (manual) {
    var noSetSelector = "#vehicle-container .vehicle-noSet";

    if (_t !== undefined) window.clearInterval(_t);

    _runVehicleInfo(manual);
