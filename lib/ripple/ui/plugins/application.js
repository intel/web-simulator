/*
 *  Copyright 2012 Intel Corporation.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

var event = require('ripple/event'),
    utils = require('ripple/utils'),
    db = require('ripple/db'),
    DB_APPLICATION_KEY = "tizen1-db-application",
    _data = {
        appList : {},
        installedAppList: {}
    },
    _appInstalledTemplate,
    _appListTemplate,
    _apps = {
        "http://tizen.org/viewer": {
            id: "http://tizen.org/viewer",
            name: "Tizen viewer",
            iconPath: "001.png",
            version: "1.9",
            show: true,
            categories: ["media"],
            size: 5120,
            packageId: "TEST_APP_ID",
            operation: "http://tizen.org/appcontrol/operation/view",
            appControl: {
                uri: "http://tizen.org/appcontrol/uri/view",
                mime: "image/*",
                category: "media",
                data: ""
            },
            type: "AUTHOR_ROOT",
            value: []
        },
        "http://tizen.org/player": {
            id: "http://tizen.org/player",
            name: "Tizen player",
            iconPath: "002.png",
            version: "2.0",
            show: true,
            categories: ["media"],
            size: 2048,
            packageId: "TEST_APP_ID",
            operation: "http://tizen.org/appcontrol/operation/play",
            appControl: {
                uri: "http://tizen.org/appcontrol/uri/play",
                mime: "video/*",
                category: "media",
                data: ""
            },
            type: "AUTHOR_SIGNER",
            value: []
        },
        "http://tizen.org/dialer": {
            id: "http://tizen.org/dialer",
            name: "Tizen dialer",
            iconPath: "002.png",
            version: "2.1",
            show: true,
            categories: ["dialer"],
            size: 2048,
            packageId: "SAMPLE_ID",
            operation: "http://tizen.org/appcontrol/operation/dial",
            appControl: {
                uri: "http://tizen.org/appcontrol/uri/dial",
                mime: "application/xml",
                category: "dialer",
                data: ""
            },
            type: "AUTHOR_ROOT",
            value: []
        },
        "http://tizen.org/sender": {
            id: "http://tizen.org/sender",
            name: "Tizen sender",
            iconPath: "005.png",
            version: "2.2",
            show: true,
            categories: ["message"],
            size: 2048,
            packageId: "SAMPLE_ID",
            operation: "http://tizen.org/appcontrol/operation/send_text",
            appControl: {
                uri: "http://tizen.org/appcontrol/uri/send_text",
                mime: "text/plain",
                category: "message",
                data: ""
            },
            type: "AUTHOR_ROOT",
            value: []
        }
    },
    _installedAppList = {
        "http://tizen.org/dialer": {
            id: "http://tizen.org/dialer",
            name: "Tizen dialer",
            iconPath: "002.png",
            version: "2.1",
            show: true,
            categories: ["dialer"],
            installDate: new Date(),
            size: 2048,
            packageId: "SAMPLE_ID",
            operation: "http://tizen.org/appcontrol/operation/dial",
            appControl: {
                uri: "http://tizen.org/appcontrol/uri/dial",
                mime: "application/xml",
                category: "dialer",
                data: ""
            },
            type: "AUTHOR_ROOT",
            value: []
        },
        "http://tizen.org/sender": {
            id: "http://tizen.org/sender",
            name: "Tizen sender",
            iconPath: "005.png",
            version: "2.2",
            show: true,
            categories: ["message"],
            installDate: new Date(),
            size: 2048,
            packageId: "SAMPLE_ID",
            operation: "http://tizen.org/appcontrol/operation/send_text",
            appControl: {
                uri: "http://tizen.org/appcontrol/uri/send_text",
                mime: "text/plain",
                category: "message",
                data: ""
            },
            type: "AUTHOR_ROOT",
            value: []
        }
    };

function _get() {
    _data = db.retrieveObject(DB_APPLICATION_KEY);
    if (_data === undefined) {
        _data = {
            appList : _apps,
            installedAppList: _installedAppList
        };
        _save();
    }
}

function _save() {
    db.saveObject(DB_APPLICATION_KEY, _data);
}

function renderAppListBox(id) {
    var item, html;
    jQuery("#application-list-box").empty();
    item = _data.appList[id];
    html = _appListTemplate.replace(/#Name/g, item.name)
            .replace(/#ID/, item.id)
            .replace(/#IconPath/, item.iconPath)
            .replace(/#Version/, item.version)
            .replace(/#Show/, item.show)
            .replace(/#Categories/, item.categories.join("<br>"))
            .replace(/#InstallDate/, item.installDate)
            .replace(/#Size/, item.size)
            .replace(/#PackageID/, item.packageId)
            .replace(/#Operation/, item.operation)
            .replace(/#URI/, item.appControl.uri)
            .replace(/#MIME/, item.appControl.mime)
            .replace(/#category/, item.appControl.category)
            .replace(/#Data/, item.appControl.data);
    jQuery("#application-list-box").html(html);
    if (_data.installedAppList[item.id]) {
        jQuery("#application-update-btn").show();
    } else {
        jQuery("#application-install-btn").show();
    }
}

function _loadAppList() {
    var nodes = jQuery("#application-list-select");
    nodes.html("");
    utils.forEach(_data.appList, function (item, index) {
        nodes.append(utils.createElement("option", {
            "value": index,
            "innerText": item.name
        }));
    });
    renderAppListBox($("#application-list-select option:selected").val());
}

function _loadInstalledAppList() {
    var html = "";
    jQuery("#application-installed-box").empty();
    utils.forEach(_data.installedAppList, function (item) {
        html += _appInstalledTemplate.replace(/#Name/g, item.name)
            .replace(/#ID/g, item.id)
            .replace(/#IconPath/, item.iconPath)
            .replace(/#Version/, item.version)
            .replace(/#Show/, item.show)
            .replace(/#Categories/, item.categories.join("<br>"))
            .replace(/#InstallDate/, item.installDate)
            .replace(/#Size/, item.size)
            .replace(/#PackageID/, item.packageId)
            .replace(/#Operation/, item.operation)
            .replace(/#URI/, item.appControl.uri)
            .replace(/#MIME/, item.appControl.mime)
            .replace(/#category/, item.appControl.category)
            .replace(/#Data/, item.appControl.data);
    });
    jQuery("#application-installed-box").accordion("destroy");
    jQuery("#application-installed-box").html(html).accordion({
        active : false,
        collapsible : true,
        autoHeight : false
    });

    $("." + "application-remove-btn").bind("click", function () {
        removeApp(this.id);
    });

}

function _displayInfo(text) {
    var html = "<b>" + text + "</b>";
    document.getElementById("application-error").innerHTML = html;
}

function _programChanged(status, id) {
    event.trigger("programChanged", [status, id]);
}

function updateApp(id, updateFlag) {
    var item = _data.appList[id];
    if (!item) {
        return;
    }
    _data.installedAppList[id] = {
        id: item.id,
        name: item.name,
        iconPath: item.iconPath,
        version: item.version,
        show: item.show,
        categories: item.categories,
        installDate: new Date(),
        size: item.size,
        packageId: item.packageId,
        operation: item.operation,
        appControl: {
            uri: item.appControl.uri,
            mime: item.appControl.mime,
            category: item.appControl.category,
            data: item.appControl.data
        },
        type: item.type,
        value: item.value
    };
    _save();
    if (updateFlag) {
        _programChanged("updated", id);
    } else {
        _programChanged("installed", id);
    }
    _loadAppList();
    _loadInstalledAppList();
}

function removeApp(id) {
    delete _data.installedAppList[id];
    _save();
    _programChanged("uninstalled", id);
    _loadAppList();
    _loadInstalledAppList();
}

event.on("appServiceReplied", function () {
    _displayInfo("The application has been launched successfully");
});

module.exports = {
    initialize: function () {
        _appListTemplate = jQuery("#application-list-template").html();
        _appInstalledTemplate = jQuery("#application-installed-template").html();
        _get();
        _loadAppList();
        _loadInstalledAppList();
        jQuery("#application-list-select").bind("focus change", function () {
            var id = $("#application-list-select option:selected").val();
            renderAppListBox(id);
        });
        jQuery("#application-install-btn").live("click", function () {
            var id = $("#application-list-select option:selected").val();
            updateApp(id, false);
        });
        jQuery("#application-update-btn").live("click", function () {
            var id = $("#application-list-select option:selected").val();
            updateApp(id, true);
        });
        jQuery("#application-update-btn").live("click", function () {
            var id = $("#application-list-select option:selected").val();
            updateApp(id, true);
        });
    }
};
