...
function _showConfigWindow() {
    ...
    $("#configuration-window-load-btn").bind("click", function () {
        ...
        //reload application prgrams
        programs = db.retrieveObject(_DB_APPLICATION_SAVE_KEY);
        db.saveObject(_DB_APPLICATION_KEY, programs);
        event.trigger("ApplicationLoad", []);
        require('ripple/ui/plugins/application').initialize();
