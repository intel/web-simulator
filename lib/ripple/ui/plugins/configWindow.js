...
var utils = require('ripple/utils'),
    ...
    deviceSetting = require('ripple/ui/plugins/deviceSettings'),
    ...
    _DIALOG_SETTING_KEY = "tizen-configuration-window-dialog-setting",
    _dialog_show_setting = 1,
    _settings = {};
...
function _showConfigWindowDoneDialog() {
    $("#configure-window-done-popup").show("slide", {direction: "up"}, "slow");
    $("#overlayBackground").show("fade", "slow");
}

function _hideConfigWindowDoneDialog() {
    $("#configure-window-done-popup").hide("slide", {direction: "up"}, "slow");
    $("#overlayBackground").hide("fade", "slow");
}
...
function _showConfigWindow() {
    ...
    $("#configuration-window-done-btn").bind("click", function () {
        ...
        _hideConfigWindow();
        _dialog_show_setting = db.retrieve(_DIALOG_SETTING_KEY);
        if (_dialog_show_setting === undefined) {
            _dialog_show_setting = 1;
        }

        if (_dialog_show_setting === 1) {
            _showConfigWindowDoneDialog();
        }
        else {
    ...
    $("#configuration-window-done-btn2").unbind('click');
    $("#configuration-window-done-btn2").bind("click", function () {
        if ($("#configure-window-done-dialog-show").is(':checked')) {
            _dialog_show_setting = 0;
        }

        db.save(_DIALOG_SETTING_KEY, _dialog_show_setting);
        _hideConfigWindowDoneDialog();
        setTimeout(reload, 500);
    });
