...
var db = require('ripple/db'),
    ...
    _cleanInputs, _loadResources, intervalId,
    _downloads;

function _get() {
    _downloads = [
        {id: "", url : "http://tizen.org/small_file.zip", size : "5", speed : "1.0", estimatedTime : "5"},
        {id: "", url : "http://tizen.org/big_file.zip", size : "20", speed : "1.0", estimatedTime : "20"},
    ];
...
function checkProperties(optionType, url, size, speed, time) {
    var back = true;
    if (url === "" || size === "" ||  speed === "" || time === "") {
...
function showUpdateNotification() {
    var url     = document.getElementById("download-url").value,
        ...
        isValid = true;
    if (url === "" || size === "" ||  speed === "" || !checkURL(url) || size <= 0 ||  speed <= 0) {
        isValid = false;
    }
    if (isValid) {
...
function _adding() {
    var id, time, url, size, speed, object, i, isValid;
    ...
    isValid = checkProperties("Add", url, size, speed, time);
    if (!isValid)
        return;
    if (_data.resources.length !== 0) {
        for (i in _data.resources) {
            if (url === _data.resources[i].url) {
                _displayInfo("Add failed, the download resource is already existed.");
                return;
            }
        }
    }
    document.getElementById("download-time").innerText = Math.round(time * 100) / 100;
    object = {'id': id, 'url': url, 'size': size, 'speed': speed, 'estimatedTime': time};
...
function _update() {
    var id, time, url, size, speed, i, isExist = false, isValid;
    ...
    isValid = checkProperties("Update", url, size, speed, time);
    if (!isValid)
        return;
    for (i in _data.resources) {
        if (_data.resources[i].url === url) {
            isExist = true;
            _data.resources[i] = {id: id, url : url, size : size, speed : speed, estimatedTime: time};
            loadResources();
            _save();
            break;
        }
