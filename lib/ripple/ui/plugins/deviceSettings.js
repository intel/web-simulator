...
module.exports = {
    initialize: function () {
        ...
        utils.forEach(settings, function (settingSection, settingType) {
            ...
            if (settingType === 'Config' || settingType === 'Cpu' || settingType === 'Storage' || settingType === 'Display' || settingType === 'Device') {
        ...
        event.on("WiFiNetworkStatusChanged", function (value) {
            document.getElementById("device-settings-WifiNetwork-status").checked = value;
        });
        event.on("CellularNetworkStatusChanged", function (value) {
            document.getElementById("device-settings-CellularNetwork-status").checked = value;
        });
    }
};
