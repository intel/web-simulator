/*
 *  Copyright 2013 Intel Corporation.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

module.exports = {
    supported: ["VehicleSpeed", "EngineSpeed", "Transmission"],
    types: ["VehicleSpeed", "EngineSpeed", "VehiclePowerMode", "TripMeter",
        "Acceleration", "Transmission", "CruiseControlStatus", "WheelBrake",
        "LightStatus", "InteriorLightStatus", "Horn", "Fuel", "EngineOil",
        "Location", "ExteriorBrightness", "Temperature", "RainSensor",
        "WindshieldWiper", "HVAC", "WindowStatus", "Sunroof",
        "ConvertibleRoof", "VehicleId", "Size", "FuelInfo",
        "VehicleType", "NightMode", "DrivingMode"],
    dataCount: 9,
    data: {
        "1384401600000": { //2013-11-14 12:00:00
            "VehicleSpeed" : {
                "vehicleSpeed": 60
            },
            "EngineSpeed" : {
                "engineSpeed": 2500
            },
            "Transmission": {
                "gearPosition": 64,
                "mode": 3
            },
            "Horn": {
                "on": false
            }
        },
        "1384402500000": { //2013-11-14 12:15:00
            "VehicleSpeed" : {
                "vehicleSpeed": 120
            },
            "EngineSpeed" : {
                "engineSpeed": 2800
            },
            "Transmission": {
                "gearPosition": 0,
                "mode": 0
            },
            "Horn": {
                "on": false
            }
        },
        "1384403400000": { //2013-11-14 12:30:00
            "VehicleSpeed" : {
                "vehicleSpeed": 180
            },
            "EngineSpeed" : {
                "engineSpeed": 3100
            },
            "Transmission": {
                "gearPosition": 128,
                "mode": 3
            },
            "Horn": {
                "on": false
            }
        },
        "1384404300000": { //2013-11-14 12:45:00
            "VehicleSpeed" : {
                "vehicleSpeed": 240
            },
            "EngineSpeed" : {
                "engineSpeed": 3400
            },
            "Transmission": {
                "gearPosition": 255,
                "mode": 2
            },
            "Horn": {
                "on": false
            }
        },
        "1384405200000": { //2013-11-14 13:00:00
            "VehicleSpeed" : {
                "vehicleSpeed": 280
            },
            "EngineSpeed" : {
                "engineSpeed": 3700
            },
            "Transmission": {
                "gearPosition": 6,
                "mode": 1
            },
            "Horn": {
                "on": false
            }
        },
        "1384406100000": { //2013-11-14 13:15:00
            "VehicleSpeed" : {
                "vehicleSpeed": 320
            },
            "EngineSpeed" : {
                "engineSpeed": 4000
            },
            "Transmission": {
                "gearPosition": 58,
                "mode": 3
            },
            "Horn": {
                "on": true
            }
        },
        "1384407000000": { //2013-11-14 13:30:00
            "VehicleSpeed" : {
                "vehicleSpeed": 220
            },
            "EngineSpeed" : {
                "engineSpeed": 3200
            },
            "Transmission": {
                "gearPosition": 255,
                "mode": 2
            },
            "Horn": {
                "on": false
            }
        },
        "1384407900000": { //2013-11-14 13:45:00
            "VehicleSpeed" : {
                "vehicleSpeed": 100
            },
            "EngineSpeed" : {
                "engineSpeed": 2600
            },
            "Transmission": {
                "gearPosition": 128,
                "mode": 1
            },
            "Horn": {
                "on": true
            }
        },
        "1384408800000": { //2013-11-14 14:00:00
            "VehicleSpeed" : {
                "vehicleSpeed": 0
            },
            "EngineSpeed" : {
                "engineSpeed": 0
            },
            "Transmission": {
                "gearPosition": 2,
                "mode": 0
            },
            "Horn": {
                "on": true
            }
        }
    }
};