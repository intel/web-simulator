...
function _initialize() {
    ...
    var projection = "EPSG:3857",
        mapStyleURI = {
            Mapnik: "http://b.tile.openstreetmap.org/${z}/${x}/${y}.png",
            Cycle: "http://b.tile.opencyclemap.org/cycle/${z}/${x}/${y}.png"
        },
        mapStyles = [], name;

    for (name in mapStyleURI) {
        mapStyles.push(new MapStyle(name, mapStyleURI[name]));
    }
    mapProviders.push(new MapProvider({name: "OpenStreetMap", connectivity: "ONLINE"}, projection, mapStyles));
}
