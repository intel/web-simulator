...
var event = require('ripple/event'),
    ...
    _security = {
        all: "http://tizen.org/api/filesystem",
        read: "http://tizen.org/api/filesystem.read", 
        write: "http://tizen.org/api/filesystem.write"
    },
    ...
    _self = {
        ...
        openStream: function (mode, onSuccess, onError, encoding) {
            function _openStream() {
                ...
                if (((_readOnly || _mode === "r") && _openMode === "w") ||
...
module.exports = {
    ...
    resolve: function (srcLocation, onSuccess, onError, accessMode) {
        function _resolve() {
            ...
            if (_mode === "r" || _mode === "rw" || _mode === "a" || _mode === "w") {
    ...
    handleSubFeatures: function (subFeatures) {
        for (var subFeature in subFeatures) {
            if (_security.read === subFeature)
                _readOnly = true;
            else if (_security.write === subFeature)
                _writeOnly = true;
        }
    }
};
