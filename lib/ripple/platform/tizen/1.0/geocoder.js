...
var db = require('ripple/db'), 
    ...
    _get, _save, _geoList_init, GeoEntry, _getProviders,
...
function GeocodeOptions() {
    var _self;
    _self = {
        maxResults : 0
    };
    _self.sortMode = new SortMode();
...
function _findCoordsByAddress(addr) {
    ...
    for (i = 0; i < _geoList.length; i++) {
        ...
        if (select === true) {
            array.push(_geoList[i].coordinate);
        }
    }
...
function GeocoderProvider(prop) {
        ...
        function _geocode() {
            ...
            ret = _pendingOperate(function () {
                ...
                if (_checkAddressType(address) === "string") {
                    ...
                    if (errorCB) {
                        setTimeout(function () {
                            errorCB(new CommonError("TYPE_MISMATCH_ERROR"));
                        }, 1);
                    }
                }
            });
...
function _getProviders() {
    if (typeof _providers === "undefined") {
        /* create default Tizen offline geo database */
        _providers = [new GeocoderProvider({name : "Tizen Database", connectivity : "OFFLINE"})];
    }
    return _providers;
}
...
_self = {
    getDefaultProvider : function () {
        if (arguments.length !== 0) {
            throw new CommonError("INVALID_ARGUMENT_ERROR", "Parameters are not void");
        }
