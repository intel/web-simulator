...
_self = function (id, name, type, security_check) {
    var msg_service = {},
        ...
        _messagingStorage = new MessageStorage(_messages, _security_check);
    ...
    event.on("MessageReceived", function (rst) {
        ...
        if (msg_utils.setMsg(msgInit, msg) === false) {
            throw (new CommonError("TYPE_MISMATCH_ERROR", "received message error"));
        }
            ...
            _sendMsg = function () {
                if (typeof _msg.id === 'string') {
                    ...
                    if (_messages.msg[_msg.id].priv.messageStatus === "DRAFT") {
                        ...
                        if (onError) {
                            setTimeout(function () {
                                onError(new CommonError("PERMISSION_DENIED_ERROR"));
                            }, 1);
                        }
                        return;
                    }
    ...
    msg_service.__defineGetter__("messagingStorage", function () {
        return _messagingStorage;
    });
    return msg_service;
};
