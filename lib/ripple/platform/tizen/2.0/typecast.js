...
function _castConstructor(constructors) {
    var vtc, isOverloaded;
    ...
    function castOverload(argv, voc, scope) {
        var iOverload;
    ...
    return isOverloaded ? castOverload : castUnique;
}
...
function _castInterface(name) {
    ...
    return function (method, argv) {
        ...
        _cast(interface[method], argv, true);
    };
}
...
(function () {
    ...
    for (i in typedef.constructor) {
        _self[i] = _castConstructor(typedef.constructor[i]);
    }
