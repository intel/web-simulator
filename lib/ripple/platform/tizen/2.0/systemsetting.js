...
function _initialize() {
    ...
    if (!_systemSettings) {
        _systemSettings = {"HOME_SCREEN": "images/home_screen.jpg", "LOCK_SCREEN": "images/lock_screen.jpg", "INCOMING_CALL": "sounds/incoming.wav", "NOTIFICATION_EMAIL": "sounds/notification.wav", "LOCALE": "eng"};
...
_self = function () {
    ...
    function setProperty(type, value, successCallback, errorCallback) {
        ...
        if (type === "LOCALE") {
            if (_data.language.indexOf(value) === -1) {
                throw new WebAPIException(errorcode.INVALID_VALUES_ERR);
            }
        }

        _systemSettings = db.retrieveObject(DBSYSTEMSETTING_KEY);
        if (_systemSettings[type] === value) 
	    return;
