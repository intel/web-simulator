...
var utils = require('ripple/utils'),
    ...
    _calendarsStorage,
    _security = {
        "http://tizen.org/privilege/calendar.read": ["getCalendars", "getDefaultCalendar", "getCalendar", "get", "find", "addChangeListener", "removeChangeListener"],
...
_self = function () {
    ...
    function getCalendars(type, successCallback, errorCallback) {
        if (!_security.all && !_security.getCalendars) {
    ...
    function getDefaultCalendar(type) {
        ...
        if (!_security.all && !_security.getDefaultCalendar) {
            throw new WebAPIException(errorcode.SECURITY_ERR);
        }
        if (!isValidType(type))
            throw new WebAPIException(errorcode.TYPE_MISMATCH_ERR);
    ...
    function getCalendar(type, id) {
        ...
        if (!_security.all && !_security.getCalendar) {
            throw new WebAPIException(errorcode.SECURITY_ERR);
        }
