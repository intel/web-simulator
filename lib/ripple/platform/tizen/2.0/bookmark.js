...
_self = function () {
    ...
    function get(parentFolder, recursive) {
        ...
        if ((parentFolder && !(new TypeCoerce(t.BookmarkFolder)).match(parentFolder)) ||
            (recursive && !(new TypeCoerce(t.boolean)).match(recursive))) {
    ...
    function add(bookmark, parentFolder) {
        ...
        if (!(new TypeCoerce(t.Bookmark)).match(bookmark) ||
            (parentFolder && !(new TypeCoerce(t.BookmarkFolder)).match(parentFolder))) {
            throw new WebAPIException(errorcode.TYPE_MISMATCH_ERR);
        }
    ...
    function remove(bookmark) {
        ...
        if (bookmark && !(new TypeCoerce(t.Bookmark)).match(bookmark)) {
            throw new WebAPIException(errorcode.TYPE_MISMATCH_ERR);
        }
