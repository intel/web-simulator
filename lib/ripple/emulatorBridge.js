...
function _getEmulatedDeviceStyle(attr) {
    var vp = document.getElementById("device-container"), value;
    value = vp.style[attr].replace(/\s/g, '');
    value = value.split("px")[0] - 4;
    return (value);
}

function _screenAvailWidth() {
    return _getEmulatedDeviceStyle("width");
}

function _screenAvailHeight() {
    return _getEmulatedDeviceStyle("height");
}
